...
unsigned int loadTexture(char const * path, bool gammaCorrection) {
    ...
    if (data) {
        GLenum format0, format1;
        if (nrComponents == 1)
            format0 = format1 = GL_RED;
        else if (nrComponents == 3) {
            format0 = gammaCorrection ? GL_SRGB : GL_RGB;
            format1 = GL_RGB;
        }
        else if (nrComponents == 4) {
            format0 = gammaCorrection ? GL_SRGB_ALPHA : GL_RGBA;
            format1 = GL_RGBA;
        }
        glad_glBindTexture(GL_TEXTURE_2D, textureID);
        glad_glTexImage2D(GL_TEXTURE_2D, 0, format0, width, height, 0, format1, GL_UNSIGNED_BYTE, data);
        glad_glGenerateMipmap(GL_TEXTURE_2D);

        glad_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, format1 == GL_RGBA ? GL_CLAMP_TO_EDGE : GL_REPEAT); // for this tutorial: use GL_CLAMP_TO_EDGE to prevent semi-transparent borders. Due to interpolation it takes texels from next repeat
        glad_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, format1 == GL_RGBA ? GL_CLAMP_TO_EDGE : GL_REPEAT);
